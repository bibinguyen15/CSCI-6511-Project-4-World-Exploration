...
def train():
        ...
        for traverse in range(traverses):
            qTable, goodStates, badStates, obstacles = model.learn(qTable, worldId=world, mode='train', learningRate=0.0001, gamma=0.9, epsilon=epsilon, goodTermStates=goodStates, badTermStates=badStates,traverse=traverse, obstacles=obstacles, runNum=runNum, verbose=v)
...
def exploit():
        ...
        for traverse in range(traverses):
            qTable, goodStates, badStates, obstacles = model.learn(qTable, worldId=world, mode="Ex", learningRate=0.0001, gamma=0.9, epsilon=epsilon, goodStates=goodStates, badStates=badStates,traverse=traverse, obstacles=obstacles, runNum=runNum, verbose=v)
